...
namespace Cs2Telegram.Commands
{
    public static class ServerCommands
    {
        ...
        public static async Task ExecuteCommandInServer(ITelegramBotClient botClient, Update update)
        {
            ...
            update.RegisterStepHandler(new StepTelegram(ExecuteCommandInServerHandler));
        ...
        public static async Task ExecuteInlineCommandInServerHandler(ITelegramBotClient botClient, Update update)
        {
            if (!(await botClient.IsAdmin(update.GetChatId())))
            {
                await CommonEvents.AccessDenied(botClient, update);
        ...
        public static async Task SendMessageToServer(ITelegramBotClient botClient, Update update)
        {
            ...
            update.RegisterStepHandler(new StepTelegram(SendMessageToServerHandler, null));
            var options = new OptionMessage();
            options.MenuReplyKeyboardMarkup = await botClient.GenerateOnlyMenu(update.GetChatId());
            Helper.SendMessage(botClient, update, msg, options);
        }
        ...
        [RequiredTypeChat(Telegram.Bot.Types.Enums.ChatType.Private)]
        [RequireTypeMessage(Telegram.Bot.Types.Enums.MessageType.Text)]
        public static async Task SendMessageToServerHandler(ITelegramBotClient botClient, Update update)
        {
            string msg = update?.Message?.Text;
            if (!string.IsNullOrWhiteSpace(msg))
            {
